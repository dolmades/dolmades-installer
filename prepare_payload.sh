...
function prepare_payload() {
	...
	echo "Pulling images..."
	START=$(date +%s.%N)
	./udocker --repo="$PAYLOAD/.dolmades/repo" pull dolmades/runtime:$DOLMADES_VERSION
	./udocker --repo="$PAYLOAD/.dolmades/repo" pull dolmades/winestable:$DOLMADES_VERSION
	./udocker --repo="$PAYLOAD/.dolmades/repo" pull dolmades/winestaging:$DOLMADES_VERSION
